...
| # | Item | Severity | Root cause / current behavior | Follow-up if needed |
|---|---|---|---|---|
| C-P25-1 | **Scatter paint is unbounded — no `downsample_for_paint` cap.** `chart/scatter.rs` draws one marker quad/path **per data point** plus an N-vertex connecting `Line`, with **no** vertex cap — unlike the line renderer, which decimates a huge series to `MAX_PAINT_VERTICES` via `downsample_for_paint`. A scatter bound to a large range therefore has **unbounded per-frame paint cost**. | Moderate (perf; not hit by any committed scene — every scatter scene is ≤ ~13 points). | Scatter has no decimation analog. The line path's extrema-based `downsample_for_paint` decimates along an **index-ordered** series; that does **not** transfer to an **unordered marker cloud** (there is no 1-D axis to decimate along, and dropping points changes the cloud). Needs its own approach (e.g. spatial/grid binning or representative sampling). | **Measure + address in P26's huge-sheet perf re-measure sweep** (the final type's cross-type perf pass). Give scatter its own cloud-aware paint cap before a large-range scatter can stress a frame. |
