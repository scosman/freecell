...
| F4 | **Border pen undo-coalescing** — several style/color nudges on one target = several undo steps | `formatting-expansion/functional_spec.md` §2 (Part 2 edge cases) | Trivial | Each border paint is its own undo entry; nudging a target's style/color N times leaves N undo steps. | No coalescing of consecutive same-target border paints (the worker coalesces text edits but border paints are applied individually). | Coalesce consecutive border paints while the popover stays on one target into a single undo entry, mirroring the edit-batch coalescing. |
