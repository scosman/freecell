...
### Charts — pie data-label fidelity (Charts P24, 2026-07-19)

The P24 pie/doughnut renderer draws on-slice **percent** labels only; the other data-label kinds and
per-point overrides are honestly badged as **Degraded**, not silently dropped. Surfaced fixing the
demo's sheet-3 "Share of Revenue by Region" pie, which carried these options and so showed the "⚠ May
not display as intended" badge (content-fixed in the bundled demo).

| # | Item | Severity | Root cause / current behavior | Follow-up if needed |
|---|---|---|---|---|
| C-P24-1 | **Pie data labels: only on-slice `showPercent` is rendered; `showVal`/`showCatName`/`showSerName`/`showLegendKey` and per-point `<c:dLbl>` overrides are not drawn — but now correctly DEGRADE.** The pie renderer (`chart/pie.rs`) paints a percentage at each slice mid-angle when `c:dLbls/showPercent` is set; it ignores the value/category/series/legend-key label kinds and any idx-keyed per-point `<c:dLbl>` override. | Mild (cosmetic label options; the visible % render is unaffected) | Excel draws whatever label kinds are enabled (often value + category name), joined by the `c:separator`, with per-point overrides; FreeCell renders only the percentage. `fidelity::unsupported_data_labels` classifies a pie showing any **non-percent** kind (`pie_shows_non_percent_label`), or carrying any per-point `<c:dLbl>` (`contains_element(xml,"dLbl")`), as **[`Degraded`]** (honest badge) rather than misleadingly omitting the labels. A percent-only pie is Faithful. **Demo content-fixed** (2026-07-19): `chart3.xml`'s pie was trimmed to `showPercent`-only (per-point `<c:dLbl>` overrides removed, non-percent toggles set to `0`), which leaves FreeCell's render unchanged (it already drew only the percent labels) and clears the badge. | Render the other pie label kinds (value/category/series-name, joined by the separator) + per-point label overrides in a later pie-label pass, then drop them from the degrade set. |
