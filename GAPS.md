...
## Engine (fork) — number-format rendering defects (2026-07-28)

Headline: **negative numbers display without their minus sign** (E6). Four defects in all, every one
of them in `base/src/formatter`, found the same way and listed together because a fork fix will keep
running into them; each still gets **its own `fix/` branch and its own upstream PR**.
...
formatter in isolation. E8/E9 came out of the round-3 *generated* corpus — codes enumerated across
the shape space `renders_faithfully` accepts, rather than a hand-written list.

| # | Bug (IronCalc) | Symptom | Our status |
|---|----------------|---------|------------|
| E6 | **`formatter::format::format_number` drops the minus sign on small magnitudes.** `is_negative = value < -(10.0_f64.powf(-(p.precision as f64)))` (`format.rs:481`) is evaluated **after** `value` has been overwritten by `to_precision(value, precision + integer_digits)` (`format.rs:438` — `:437` is the comment line above it), so the test is not "is the value negative" but "is the *pre-rounded* value below `10^-decimals`". Everything at or above that bound keeps its sign; everything below silently loses it. `get_formatted_cell_value` (`model.rs:2815` → `:2826`) calls exactly this, so it is what every cell shows. | **A cell formatted `#,##0` holding −1 displays `1`.** The threshold is **≈`10^-decimals`**, *not* `1.5 × 10^-decimals` — those two coincide only at `decimals = 0`, which is why the earlier write-up (and this row) got it wrong. Exactly, it is `10^-d + 5×10^-(2d+1)` (the pre-round keeps `d+1` significant digits). **Measured by bisection:** `0` → **1.5**; `0.0` → **0.105**; `0.00` → **0.01005**; `0.000` → **0.0010005**; `0%` → **0.015** (the ×100 percent scale moves the band, not the rule). So `#,##0` shows −1 as `1`, and `0.00` shows −0.005 as `0.01`. `General` is unaffected (a separate code path). Silent numeric misinformation on the **primary display surface**, worst on the most common format in a spreadsheet — a variance column of small negatives reads as if every value were positive, while formulas over the same cells stay correct. | **Filed, unfixed.** Carved out of the differential gate by a named predicate (`is_ironcalc_sign_bug`) so the disagreement is recorded rather than hidden — `chart-model` is **correct** here, so the chart must not be "fixed" to match the cell. The predicate additionally requires the chart's unsigned rendering to carry a **non-zero digit**: without that clause it also swallowed 15 pairs where `chart-model` was the buggy side (it printed `-0`/`-0.00` for negatives that round to zero — fixed 2026-07-28). **Round 4:** the predicate no longer shape-matches. Its `!cell.contains('-')` clause was not a statement about the sign at all — it excluded every genuine E6 pair whose *format code* carries a literal `-` (`-0`, `0-`, `$-0` on −1), which the widened corpus now reaches; the sign's position is pinned by `chart[1..] == cell` instead. In its place the carve-out **reconstructs IronCalc's own `is_negative`** (`format.rs:438` pre-round, then `:481`) and fires only when that reconstruction says "sign dropped" — and the reconstruction is itself asserted against IronCalc's measured behaviour on every pair the gate sees, so a fork fix that moves the band fails there first. Deleting the carve-out when the fork lands the fix tightens the gate automatically. |
| E7 | **The same function does not implement a single rounding rule; it rounds in pieces and the pieces disagree.** (1) It pre-rounds with `to_precision(value, precision + integer_digits)` (`format.rs:438`; `:437` is the comment line above it) — that many **significant** digits, via Rust's `{:.*e}`, which is half-to-**even**; then (2) it renders the integer part with `value_abs.round()` when `precision == 0` (half **away from zero**) or `value_abs.floor()` plus a *separately rounded* fractional string (`get_fract_part`) when `precision > 0`. **The mechanism is ONE thing, not a list of cases** — and both earlier characterisations ("half-to-even except 0.5", then "two bands") were narrower than it: **for `\|v\| < 1` the integer part is `"0"`, so the pre-round keeps `decimals + 1` significant digits and the render then rounds *again* to `decimals` decimals — a plain double round. Anything that ties or crosses a tie at the intermediate precision is corrupted.** For `\|v\| ≥ 1` the pre-round happens at exactly the rendered precision and nothing is corrupted. | The two "bands" named before are **sub-cases**, not the extent: **(a) `decimals == 0`, `\|v\| ∈ [0.45, 0.5]`** — one significant digit survives, the band becomes `0.5` and `round()` (half away from zero) makes it **`"1"`** (correct: `"0"`); **(b) lost fractional carry, `decimals ≥ 1`** — `get_fract_part` rounds the fraction separately and slices `"0.ddd"[2..]`, so a fraction that rounds up to `1.0` yields an empty slice and the carry is discarded (**`0.96` under `0.0` displays `"0.0"`**). Neither explains `0.855` under `0.0` (cell `0.8`, chart `0.9`), `0.0495` under `0.0` (cell `0.1`, chart `0.0`) or `-0.8745` under `0.00` (cell `-0.88`, chart `-0.87`) — **886 pairs** matched neither sub-case, and a decimal-exact reference put `chart-model` on the right side of every one. Separately, for `\|v\| ≥ 1` IronCalc lands on half-to-**even** (`2.5 → "2"`, `1234.5 → "1234"`) where **Excel is half-away-from-zero** (`3`, `1235`) — wrong, but consistently so. | **Filed, unfixed.** Lower severity than E6. Its own `fix/` branch + PR if it proves independent of E6 (both live in `format.rs`'s `ParsePart::Number` arm, so they may not be). Carved out as `is_ironcalc_rounding_defect(code, value, cell)`, now derived **from the mechanism**: it reconstructs IronCalc's sub-unit pipeline (pre-round to `decimals + 1` significant digits, then the separate fraction round with the carry dropped) and fires only when that reconstruction reproduces the cell **byte for byte**. Because the reconstruction is rendered through `chart-model` itself, it can only match when `chart-model`'s *presentation* already equals IronCalc's and rounding is the sole difference. **That argument was asserted in four places and was FALSE for the one case it was written to cover — sign suppression** (round 4, measured): the reconstruction renders the *positive* predicted magnitude and its negative fallback renders `-predicted`, which for the whole lost-carry class is `-0.0`, and `-0.0 < 0.0` is `false`. Re-introducing the round-2 chart-side defect verbatim therefore left all six tests green, with 6 424 pairs silently absorbed (`211156 agree / 8008 E7` → `204732 / 14432`). It is now enforced rather than argued: the carve-out may not fire unless the chart and the cell made the **same sign decision**, each measured by re-rendering the same magnitude rather than by looking for a leading `-` (a code may carry a literal `-` of its own). Under that gate the same mutation turns the suite red. It cannot fire at all for `\|v\| ≥ 1`. **Knock-on:** `chart-model`'s `{:.n}` half-to-even is pinned to IronCalc's behaviour on purpose so the axis matches the cell beside it; when the fork fix lands, half-away-from-zero becomes correct in `chart-model` too and `numfmt::format_magnitude` must be revisited with it. |
| E8 | **Thousands grouping is positioned from the *token* index, not the digit index.** In `format.rs`'s `ln <= digit_count` branch the separator test is `use_group_separator(p.use_thousands, ln - digit_index, …)` where `digit_index` counts integer digit **tokens** consumed (including the leading `#` ones that print nothing) while `ln` is the value's actual digit count. The two coincide only when `ln == digit_count`. The same branch also emits the zeros it padded in for required `0` tokens **without** ever consulting the separator. | **`##,##0` on 1234.5 renders `1234.5` in a cell** where Excel shows `1,234.5` — the separator vanishes whenever `3 < ln < digit_count`, i.e. for any grouping code with **five or more** integer digit tokens. And **`0,000` on 5 renders `0005`** where Excel shows `0,005` — padded zeros are never grouped. Everyday grouping codes (`#,##0`, `#,###`, `$#,##0.00`, `#,#00`) are unaffected: they have four integer digit tokens and at most three required zeros, which is provably out of reach of both. | **Filed, unfixed. Narrowed rather than carved out.** `renders_faithfully` now **rejects** a grouping code with more than four integer digit tokens or more than three required zeros, so such a chart is badged ⚠ instead of silently disagreeing — `chart-model` groups the way Excel does and must not be "fixed" to match. The bound is derived, not fitted: agreement requires `ln == digit_count`, `ln ≤ 3`, or `ln > digit_count`, and "at most four tokens" is exactly the condition that makes `3 < ln < digit_count` unreachable. Fork fix ⇒ widen the predicate back. **What is and is not confirmed (round 4):** the *token-index* half is independently verified against IronCalc — `###,##0` on 12345 gives cell `1,2345` vs chart `12,345`, `00,000` on 1234 gives cell `0,1234` vs chart `01,234`, every accepted grouping code agrees at every corpus value, and every mis-grouping code is rejected. The *padded-zeros* half (`0,000` on 5 → Excel `0,005`) rests on a reading of ECMA-376 §18.8.31 and **has not been checked against a real Excel**; IronCalc's `0005` is confirmed, Excel's `0,005` is inferred. The predicate rejects the shape either way, so nothing depends on resolving it — but do not cite it as measured. |
| E9 | **The lexer reads a doubled quote as an escaped quote inside one string literal.** `consume_string` treats `""` the CSV way: `0"a""b"` lexes as the single literal `a"b`, not as two adjacent literals `ab`. ECMA-376 does not define the escape, and Excel's own handling of adjacent quoted literals is not something this repo can settle from the spec. | A code carrying `""` renders a stray `"` in the cell (`#.0"%"" kg"` on 1 → `1.0%" kg`) that the chart does not. Vanishingly rare in real files — no fixture in-tree has one. | **Filed, unfixed, low priority.** `renders_faithfully` **rejects** any code containing `""` rather than picking a side, so the chart degrades honestly. Worth raising upstream as a grammar question before writing a fix. |
...
|---|---|---|---|---|
| C-G5-1 | **Inserting a chart onto a sheet that already carries a LOADED chart fails the save with a `SaveError`.** Confirmed at HEAD (v05-cleanup-1 / G5, 2026-07-28); the v0.5 `dLbls` half of that unit is fixed, this half is **v1.0**. | Moderate — **fails loudly, never corrupts.** The save is refused with "authored charts couldn't be saved"; no silent drop, no double `<drawing>`, and the original file is untouched (atomic save). The user's workaround is to put the new chart on a different sheet. It cannot happen at all in a workbook that had no charts to begin with. | The two save modes cannot compose on one sheet's drawing. `worker::run`'s save runs mode 1/2 (`reinject_live_charts` — byte-preserve/patch the original's chart + drawing parts) and then mode 3 (`write_authored_charts` — synthesize from the model) on top. `write_authored_charts` has a fail-loud precondition that the target sheet has no `<drawing>` yet (`chart::write`, test `write_fails_loudly_on_sheet_with_existing_drawing`), which the re-injected loaded drawing has already violated. Two authored charts on one sheet DO share a drawing correctly (`write_two_authored_charts_on_one_sheet_share_a_drawing`) — it is specifically loaded + authored on the same sheet that cannot merge. | Teach `write_authored_charts` to **merge** an authored anchor into an existing `xl/drawings/drawingN.xml` (append the `xdr:twoCellAnchor`, add the chart part + its rel to that drawing's `_rels`, extend `[Content_Types]`) instead of refusing when one is present. Needs care with the re-inject source advance: `chart_source_path` deliberately does not advance when authored charts exist, because `reinject` carries `xl/charts/*` + `xl/drawings/*` by prefix and would then duplicate what mode 3 re-synthesizes. |
| C-G5-2 | **A valueless OOXML boolean (`<c:showVal/>`) is read as FALSE, but `CT_Boolean/@val` defaults to `true`.** Found by the v05-cleanup-1 / G5 code review (2026-07-28) while removing the *second* spelling of this predicate from the save path. **v0.5.** | Mild — needs a file that omits `@val` on a `c:dLbls` `show*` element. Excel always writes `val`, so this is only reachable from a hand-written/third-party generator. Nothing in the repo's fixtures hits it. | `chart::load::child_bool` is `matches!(child_val(node, name), Some("1") \| Some("true"))`, which conflates "the element is absent" (correctly false) with "the element is present but carries no `val`" (should be **true** per `CT_Boolean`'s `@val` default). Deliberately **not** fixed inside a save-side unit: the change is in the loader, would alter what `parse_chart_xml` returns for such a file, and had no bearing on the G5 data loss. The save side no longer has a competing spelling — `patch_data_labels` now removes any `c:delete` unconditionally (schema: `delete` and the settings group are mutually exclusive), so no truthiness test remains there. **But the save side is no longer neutral either (added by the 2026-07-28 re-review):** the same phase made `patch_data_labels` skip an **unchanged** modelled child instead of rewriting it, which *persists* the misread. Before, every modelled child was regenerated on every save, so a file spelling `<c:showVal/>` converged to `val="0"` and the divergence died at the next save; now, toggling an unrelated field leaves that element byte-identical, so the saved file keeps showing values in Excel while FreeCell shows them off. A read-only bug became a **persisted** disagreement between the file and the UI — which is the real argument for fixing `child_bool` rather than waiting for a file that hits it. | Rewrite as `child(node, name).map_or(false, \|n\| attr(&n, "val").map_or(true, is_truthy))` in `load.rs`, with a `<c:showVal/>` test — a few lines, in a loader-scoped change where the behaviour delta can be reviewed on its own. |
| C-G5-3 | **Turning a series' data labels fully off discards that `c:dLbls`' unknown children** (label typography `c:txPr`, fill `c:spPr`, per-point `c:dLbl` overrides, `c:extLst`). Recorded with the v05-cleanup-1 / G5 fix (2026-07-28). **v1.0** (cosmetic residue of an intentional destructive edit). | Mild — the user asked for the labels to be gone, and nothing that still renders is lost. But two corrections from the 2026-07-28 re-review: (1) what is discarded is **not only styling** — a per-point `c:dLbl` may carry `c:tx`/`c:rich`, i.e. **user-authored label text** (the phase fixture calls it "a label the user re-typed"), so this can destroy *content*, not just formatting; (2) it can fire on a **semantically no-op** edit — see the root cause. Note the *values* the model can't represent (`dLblPos val="bestFit"`, `numFmt formatCode="General"`, `numFmt@sourceLinked`) **do** survive every other label edit — that is the fixed part. | `chart::save::patch_data_labels` writes `<c:dLbls><c:delete val="1"/></c:dLbls>` on a clear, replacing the node whole. It cannot keep the settings-group children (`CT_DLbls` is `dLbl*, (delete \| Group_DLbls), extLst?` — they are illegal beside a `delete`), and keeping the node's *other* children is not free either: it would have to preserve `dLbl*`/`extLst` only, in order, around the inserted `delete`. Also note the round-trip is now asymmetric by design: a cleared `Some(labels)` → `None` re-opens as `Some(all-off)` (the loader does not read `c:delete`), which renders identically (`is_shown()` is false). **Trigger, precisely (2026-07-28 re-review):** it does not need the user to turn anything off. The worker sends `None` for *any* all-off toggle set (`labels.is_shown().then_some(labels)`), the call is gated only on `series.data_labels != cached_series.data_labels` — and `None != Some(all-off)` — so a `c:dLbls` that was **already** all-off — while carrying a per-point `c:dLbl`, `c:txPr` and `c:extLst` — is replaced by a bare `c:delete` on an edit that changes nothing semantically. | If label styling must survive an off→on cycle, keep the leading `c:dLbl*` and trailing `c:extLst` around the inserted `c:delete` (both are schema-legal there), and/or teach `load::read_data_labels` to map a truthy `c:delete` to `None` so the model round-trips exactly. **The cheap first step — considered, deliberately not taken here:** skip the clear entirely when `cached.is_shown()` is already false, which kills the no-op-edit case for ~two lines. It is not free, so it wants its own change rather than a review-round drive-by: (a) it drops the normalization that today converts an already-off `c:dLbls` into the canonical `c:delete` form, so `patch_data_labels_clearing_writes_an_explicit_delete` would have to be re-premised on a *shown* fixture; and (b) it makes the C-G5-2 misread load-bearing on the save path — a file whose `<c:showVal/>` reads as false would be treated as "already off" and left alone. Fix `child_bool` (C-G5-2) first, then add the guard. |
...
## Conditional Formatting — first pass shipped; deferred behaviors (2026-07-17)

The `specs/projects/conditional-formatting` **first pass** ships **highlight (classic) rules +
color scales** with **full rule management** (list / add / edit / delete / reorder priority), a
right-docked sidebar (built on a reusable `docked_sidebar` container extracted from the chart
panel), value-dependent **live** rendering (fills/font/color re-evaluate as values change), and
`.xlsx` **round-trip**. The engine (IronCalc fork `freecell-fixes`) already models the whole
feature; the parts below were **deliberately deferred**. The three visual families (CF1–CF3) are
**planned as P11–P13 of that same project** — each needs a new in-cell grid draw primitive; the
rest are authoring / fidelity boundaries. A **loaded** rule of any deferred kind still lists in the
sidebar as a non-editable **Badge** (deletable) and never corrupts its cell (it renders the base
style; the decoration is simply not drawn).

| # | Deferred behavior | Spec | Severity | Current behavior | Root cause / home |
|---|---|---|---|---|---|
| CF1 | **Data bars** (in-cell horizontal bars) — not authored or rendered | `functional_spec.md §9` | Moderate | Loaded data-bar rule → Badge; its cell renders base style (no bar); not authorable | `ExtendedStyle.data_bar` dropped in `extended_render_style`; needs a grid in-cell bar primitive + editor. **Planned P11.** |
| CF2 | **Icon sets** — not authored or rendered | `functional_spec.md §9` | Moderate | Loaded icon-set rule → Badge; base-style render (no glyph); not authorable | `ExtendedStyle.icon` dropped; needs an in-cell icon-glyph primitive (engine `Icon` enum → lucide) + editor. **Planned P12.** |
| CF3 | **Icon ratings** (repeated glyphs) — not authored or rendered | `functional_spec.md §9` | Mild | Loaded rating rule → Badge; base-style render; not authorable | `ExtendedStyle.rating` dropped; repeated-glyph primitive + editor. **Planned P13.** |
| CF4 | **`TimePeriod` date-range variants** (Between / NotBetween, explicit date1/date2) not authorable | `functional_spec.md §9` | Mild | Parameterless date periods (Today/Yesterday/Last7Days/This-Month/…) ARE authorable; a loaded Between/NotBetween date rule → Badge | `CfRuleSpec`/editor omit the two-date operands; add a date-range row + map to `CfRuleInput::TimePeriod{date1,date2}`. |
| CF5 | **Color-scale `Formula` thresholds** not authorable | `functional_spec.md §9` | Mild | Min/Max/Number/Percent/Percentile stops authorable; a loaded scale with a `Cfvo::Formula` stop → Badge | `CfThresholdKind` has no `Formula` variant (intentional); add one + a formula input. |
| CF6 | **Theme-colored color scales** treated as non-editable | `functional_spec.md §9` (P1 CR fix) | Mild | An imported scale whose stops use a **theme** color (not concrete RGB) → Badge (deletable, not editable) | `Color::Theme`/`None` can't round-trip through the editor's `Rgb` stop without coercion to black (would clobber the file's colors on save); deferred to preserve fidelity. |
| CF7 | **Format-editor attributes** limited to fill / text-color / bold / italic | `functional_spec.md §4, §9` | Mild | The rule format editor exposes only those four `Dxf` attrs — underline, strikethrough, border, number-format override, alignment are not authorable. **Load asymmetry (P3):** a rule loaded from a file whose `Dxf` carries underline/strike/alignment DOES apply them (they ride `ExtendedStyle.style`), while a loaded `Dxf` number-format / border is dropped by the render cache — an inconsistent subset. | `CfFormat` models only the four; the render cache overwrites num_fmt/border from the base style. Widen `CfFormat` + editor (and the cache overlay) to close both the authoring gap and the load asymmetry. |
| CF8 | **CF cache invalidation rebuilds the whole sheet** on any value change | `architecture.md §4.4` | Mild (perf) | For a sheet with any CF rule, every recompute rebuilds that sheet's entire style cache via the extended read (needed because Top-N/average/color-scale are range-global); **non-CF sheets pay nothing** | Correct-but-coarse; a dirty-range-scoped rebuild is the follow-up if a CF-heavy sheet shows edit-path cost. |
| CF9 | **Fork fix (CF-undo-of-sheet-delete) not pinned** — push blocked | — | Low | IronCalc's `DeleteSheet` undo didn't restore `conditional_formatting`; FreeCell's reconcile-on-undo is in place but has nothing to republish until the fork fix ships (one gated `#[ignore]`d test) | Push to `scosman/ironcalc` is 403-denied this session. Fix + test preserved as a patch → [`specs/projects/conditional-formatting/fork-fixes/`](specs/projects/conditional-formatting/fork-fixes/) (apply → `cargo update -p ironcalc_base -p ironcalc` → un-ignore the test). |

**Tier note:** the shipped first pass is the bulk of the **v0.5** "Conditional formatting" row below.
**Data bars / icon sets / ratings** (CF1–CF3) are the natural **v1.0** completion; the
authoring/fidelity residuals (CF4–CF8) are v1.0 polish; CF9 is a fork-pin chore.
...
|---|---|---|
| **Negative numbers display without their minus sign** (engine) — a cell formatted `#,##0` holding **−1 shows `1`** | Above ([Engine (fork) — number-format rendering defects](#engine-fork--number-format-rendering-defects-2026-07-28), E6–E9) | **Correctness, not a feature gap — arguably the first thing on this list.** `format_number` drops the sign whenever the pre-rounded magnitude is at or below **`10^-decimals`** (measured: 1.5 at `0`, 0.105 at `0.0`, 0.01005 at `0.00` — *not* the `1.5 × 10^-decimals` this row used to claim, which is only right at zero decimals), so the most common format in a spreadsheet shows the wrong number with no indication. **Fork fix** (one `fix/` branch, one upstream PR); do **not** patch FreeCell's display path. A differential test already pins it and will tighten automatically when the fix lands. |
...
| **Hide / unhide rows & columns** | **Shipped v0.5** (header context-menu Hide/Unhide; zero-size render; round-trips to `.xlsx`; `gaps_closing_7_15` Phase 5) | **CORRECTION:** the earlier note ("engine `Row.hidden` has no `UserModel` setter; `Col` has no hidden field at all") was stale — it predated upstream commit `a520f48f` "Adds hide/show row/column to the API", which the pinned fork rev (`81feec4`/`freecell-fixes`) **already contains**: both `Row.hidden` **and** `Col.hidden` fields, xlsx import-parse + export-emit for both, and undoable `set_rows_hidden`/`set_columns_hidden`. So **no fork work was needed** this round — the whole feature is FreeCell-side (cache hidden-set + zero-size axis, header menu, worker command). Follow-on (tracked): a thick-divider marker between non-adjacent headers (D4.2). |
...
| **Hyperlinks** (open, create/edit, preserve) **+ the `HYPERLINK()` function** | **NEW** (checked: engine neither imports nor exports hyperlinks; `HYPERLINK` has no `Function` variant or eval arm in the pinned fork) | Common in home sheets (link lists, indexes). Fork modelling + UI (click-to-open, ⌘K to add). `HYPERLINK()` was deliberately deferred out of `scalar-functions-batch` to travel with this row — it can compute a display value but is near-useless until links are clickable. |
...
| **Conditional formatting — completion** (data bars, icon sets, ratings; + date-range/formula/theme-color authoring residuals) | Above ([CF deferred-behaviors section](#conditional-formatting--first-pass-shipped-deferred-behaviors-2026-07-17), CF1–CF8) | The v0.5 first pass shipped highlight rules + color scales + full management + live render + round-trip. This closes the three in-cell **decoration** families (each a new grid draw primitive — planned project phases P11–P13) plus the authoring/fidelity residuals (CF4–CF8). Engine already models all of it. |
| **Custom number-format editor** (format-code entry w/ live preview) | **NEW** | Engine already renders arbitrary codes — UI-only. |
| **Fraction number format** (`?/?`, `# ?/?`, `??/??`) | **NEW** (deferred from the v0.5 preset-breadth batch, 2026-07-13) | IronCalc's `?/?` fraction display is effectively unimplemented — renders garbled output (not `#VALUE!`), so the preset was dropped from the v0.5 dropdown rather than ship broken. Needs a fork implementation of fraction formatting (upstreamed per policy). [`projects/fraction-number-format.md`](projects/fraction-number-format.md) |
...
| **Open dialog not filtered to `.xlsx`/`.csv`** (the picker lists every file; Excel's filters to workbook types) | **NEW as a tier row** (2026-08-07 — previously logged inline as an `mvp-gaps` UI-review note) | **Graceful today, not broken:** a files-only picker routes by the pick — a `.csv` imports as an untitled workbook, anything else runs the `.xlsx` loader whose **post-selection** magic-byte check rejects a non-workbook (`LoadError::NotXlsx` → the "Couldn't open the workbook" dialog). No crash, no wrong-file load. (There is intentionally no separate "Import CSV…" menu item — the **Open** panel handles a `.csv` pick.) **Blocked on the pinned dep:** the pinned gpui rev's `PathPromptOptions` (`crates/gpui/src/platform.rs`) has **no** extension/content-type field, and neither the macOS impl (`NSOpenPanel`, never calls `setAllowedContentTypes:`) nor the Linux `prompt_for_paths` exposes a filter hook. Fix once gpui is bumped to a rev whose path prompt gains a filter field (or via a fork fix to `PathPromptOptions`): set the allowed set to **`.xlsx` + `.csv`** in `open_panel_options` (`shell/window.rs`). |
...
| **More chart types** (radar, stock, histogram/Pareto, waterfall, treemap, sunburst, funnel, box & whisker; true 3-D render — today 3-D degrades to 2-D honestly) | **NEW** (checked: `ChartKind` = bar/line/area/pie/scatter/bubble; unsupported kinds → placeholder, 3-D → degraded 2-D) | Owner-named v2.0. |
| **Combo / dual-axis charts** (G1b — multiple chart groups in one plot area, per-group types, secondary value axis) | Above (v05-cleanup-1 / G1, 2026-07-28) | **Behaviour, corrected in G1** — this row previously implied a combo gets the "unsupported chart" placeholder like radar or stock. **It does not.** `parse_chart_xml` keeps only the **first *recognized*** chart-group element in `c:plotArea` and discards the rest, so an ordinary Excel bar+line loads as bars only, with the line series **absent from the picture**. ("Recognized", not literally first: a group with no `ChartKind` is skipped over, so `radarChart` + `barChart` parses the *bar* — though that pairing is classified `Unsupported` and shows the placeholder regardless.) Until G1 a combo was also classified `Faithful` and drawn with no badge; G1 makes `source_fidelity` count chart groups and return `Degraded`, and `parse_chart_xml` emits a `tracing::warn!` naming the retained and dropped groups. The loss is now **disclosed** — honest, not correct. **v2.0.** Bar+line with a secondary axis is an ordinary Excel construct, not an exotic one, so this is a real feature gap rather than a fidelity footnote — it should lead the chart-types work. Needs: multiple groups per plot area, per-group kinds, and secondary `c:valAx` support end to end (parse → model → render → save). |
