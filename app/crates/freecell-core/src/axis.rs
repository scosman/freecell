...
//!
//! **Ported from** `experiments/04-ui-poc/poc-core/src/layout.rs` (the validated POC),
//! copied not path-referenced per `architecture.md §1`. Two adaptations for the app:
//! - the sizer bound gains `Send + Sync` so an `Arc<Axis>` can live in the
//!   worker-written / UI-read resident cache (`architecture.md §2, §6`), and
//! - [`Axis::from_overrides`] builds the "default size + sparse per-track overrides"
//!   axis the geometry cache needs, without materializing anything per track.
//!
//! **Why not a plain prefix-sum array?** The Excel-max grid is 1,048,576 rows ×
//! 16,384 cols. A full cumulative-size array over 1M+ rows would be tens of MB and cost
//! O(n) to build. Instead this uses a **two-level segment sum**: cumulative size at each
//! *block* boundary (block size [`BLOCK`]), then a lookup binary-searches the O(n/B) block
//! sums and does a short O(B) scan inside the landing block. Build is O(n) additions but
//! O(n/B) memory; `offset_of` / `index_at` are O(log(n/B) + B). Sizes come from a
//! per-index sizer closure, so nothing per-cell is materialized.
//!
//! All arithmetic that accumulates across the whole axis is done in `f64` to avoid `f32`
//! precision loss over ~10^5–10^6 cells; individual sizes stay `f32`.
...
pub struct Axis {
    count: u32,
    /// Cumulative size (px) at the *start* of each block: `block_starts[b]` is the sum of
    /// the sizes of all indices before `b * BLOCK`. Length is `num_blocks + 1`, so the
    /// last entry is the total axis size.
    block_starts: Vec<f64>,
    /// Per-index size in px. `Send + Sync` so the axis can be shared across the worker and
    /// UI threads inside the resident cache.
    sizer: Box<dyn Fn(u32) -> f32 + Send + Sync>,
}

impl Axis {
    /// Builds an axis of `count` tracks whose sizes come from `sizer`.
    pub fn new(count: u32, sizer: impl Fn(u32) -> f32 + Send + Sync + 'static) -> Self {
    ...
    /// Builds an axis from a uniform `default_px` size plus sparse per-track `overrides`
    /// (px) — the shape the geometry cache holds after unit-converting file values once
    /// (`components/style_cache.md`). The overrides map is shared into the sizer, so the
    /// axis stays O(overrides) memory, not O(count).
    pub fn from_overrides(count: u32, default_px: f32, overrides: BTreeMap<u32, f32>) -> Self {
        let overrides = Arc::new(overrides);
        Axis::new(count, move |i| {
            overrides.get(&i).copied().unwrap_or(default_px)
        })
    }
    ...
    /// The first track index whose *start* is at or after `offset` px — equivalently, the
    /// index of the track containing `offset` (its start `<= offset < end`). Clamped to
    /// `[0, count]`. `index_at(total())` returns `count`.
    ...
    /// The half-open range of track indices visible in a viewport of `extent` px starting
    /// at scroll offset `scroll`, expanded by `overscan` tracks on each side and clamped to
    /// `[0, count)`.
...
/// Compile-time guard for the phase's headline invariant: `Axis` MUST stay `Send + Sync` so
/// an `Arc<Axis>` can live in the worker-written / UI-read `Arc<RwLock<SheetCaches>>`
/// (`architecture.md §2, §6`). This is the entire reason the sizer bound carries
/// `Send + Sync`; if that bound is ever dropped, or a non-`Sync` value captured in a sizer,
/// this fails to compile instead of silently breaking downstream in Phase 6.
const _: fn() = || {
...
mod tests {
    ...
    fn offset_and_index_roundtrip() {
        ...
        for i in [0u32, 1, 6, 7, 8, 511, 512, 513, 2500, 4999] {
            ...
            // The track starting exactly at `off` is `i` (index_at returns the track whose
            // start <= off < end).
    ...
    fn visible_range_covers_viewport_and_clamps() {
        ...
        // Covers the viewport: the first data track fully covering `scroll` is inside the
        // range, and the track covering `scroll + extent` is inside too.
    ...
    fn handles_excel_max_rows_without_oom() {
        // 1,048,576 rows. The block-sum vector is ~2,048 f64 (~16 KB), NOT a per-row array.
        // Build + query near the end must be fast and correct.
        let count = limits::MAX_ROWS;
    ...
    #[test]
    fn from_overrides_matches_default_and_override() {
        let mut overrides = BTreeMap::new();
        overrides.insert(2u32, 50.0f32);
        overrides.insert(9u32, 7.0f32);
        let axis = Axis::from_overrides(10, 20.0, overrides);
        // Overridden tracks report their override; the rest report the default.
        assert_eq!(axis.size_of(0), 20.0);
        assert_eq!(axis.size_of(2), 50.0);
        assert_eq!(axis.size_of(9), 7.0);
        // Total = 8 defaults (20) + 50 + 7 = 217.
        assert!((axis.total() - (8.0 * 20.0 + 50.0 + 7.0)).abs() < 1e-6);
        // The override at index 2 shifts every later offset.
        assert!((axis.offset_of(3) - (2.0 * 20.0 + 50.0)).abs() < 1e-6);
    }
}
