...
/// The IronCalc-space row height a cell at `font_px` (device px) auto-grows to on a font-size
/// change, keeping the **default** row-height : font-size ratio
/// ([`DEFAULT_ROW_HEIGHT_PX`] : [`DEFAULT_CELL_FONT_PX`] ≈ 1.85) at every size. The grow is
/// proportional, not "line box + fixed padding": at the default 13 px font it yields exactly the
/// 24 px default row, and a 20 pt (≈26.7 px) font grows the row to ≈49 px device — always enough
/// to contain the font's line box (gpui's default ≈1.618× the glyph) with the same proportional
/// slack the default cell has. That slack is what keeps vertical alignment (top/middle/bottom)
/// meaningful: the old fixed-padding formula grew the row *less* than the line box for large fonts,
/// so the overflowing line box inverted top/bottom placement. The result is returned in IronCalc px
/// (the engine's storage space); the render path scales it back to device via [`row_px`], so we
/// pre-divide by that factor here.
...
mod tests {
    ...
    fn autofit_row_keeps_default_ratio() {
        // The font-size auto-grow height is PROPORTIONAL: at the default font it is exactly the
