...
//! with `gpui::PathBuilder`, using the shared cumulative math in [`crate::stacking`].
//!
//! It reuses the [`ScalePoint`] category axis + one shared [`NiceScale`] value axis pattern
//! from [`crate::line`], and the shared title/axis-title/legend chrome from [`crate::chrome`].
...
use chart_model::{Chart, ChartKind, Grouping, SeriesData};

use crate::chrome::chart_frame;
use crate::palette::series_color;
use crate::stacking::{category_totals, percent_segments, stacked_segments, Segment};
...
mod tests {
    use super::*;
    use chart_model::{Axis, Category, Legend, Series};
