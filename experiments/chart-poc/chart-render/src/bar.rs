...
//! series, a stacked slot holds one column of cumulative segments (via [`crate::stacking`]).
...
//!
//! Colors are explicit (see [`crate::style`]) so the headless capture is deterministic
...
use chart_model::{BarDir, Chart, ChartKind, Grouping, SeriesData};
