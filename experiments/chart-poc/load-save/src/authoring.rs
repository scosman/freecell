...
/// A `c:title` block with rich text (`a:t`) — the shape [`crate::load::parse_title`] reads.
...
mod tests {
    use super::*;
    use crate::load::{load_charts_from_xlsx, parse_chart_xml};
    use chart_model::{BarDir, ChartKind, Grouping, SeriesData};
