...
//! `.xlsx` zip and parse each embedded chart's XML into the [`chart_model::Chart`] model,
...
/// Loads every embedded chart in the workbook into [`chart_model::Chart`], in
...
fn parse_series_color(ser: &Node) -> Option<chart_model::Color> {
    ...
    Some(chart_model::Color::from_hex(v))
}
 ...
 <c:chart>
  ...
  <c:legend><c:legendPos val="r"/></c:legend>
    ...
    fn parses_column_chart_kind_values_and_chrome() {
        ...
        assert_eq!(s.color, Some(chart_model::Color::from_hex(0x4472C4)));
        match &s.data {
